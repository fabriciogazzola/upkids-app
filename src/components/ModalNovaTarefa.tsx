    ...
    <div className="fixed inset-0 bg-blue-900/60 backdrop-blur-md flex items-center justify-center p-4 z-[9999]">
      ...
      >
        ...
        <div className="space-y-5">
          {/* Campo Descrição */}
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-1 block">O que precisa ser feito?</label>
            <input 
              type="text"
              value={descricao}
          ...
          {/* Campo Pontos */}
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-1 block">Quantos pontos vale?</label>
            <input 
              type="number" 
              value={pontos}
              placeholder="Ex: 50"
          ...
          {/* Seleção de Filhos */}
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-2 block">Para quais heróis?</label>
            <div className="flex gap-2">
              {filhos.map((f: any) => (
                <button
                  ...
                  className={`flex-1 py-3 rounded-xl font-black text-xs transition-all border-2 ${selecionados.includes(f.id) ? 'bg-blue-600 border-blue-600 text-white' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
                >
                  {f.nome.toUpperCase()}
                </button>
              ))}
          ...
          {/* Seleção de Dias */}
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-2 block">Em quais dias?</label>
            <div className="flex justify-between">
              {diasMapa.map((d) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full font-black text-[10px] transition-all flex items-center justify-center border-2 ${dias.includes(d.id) ? 'bg-yellow-400 border-yellow-500 text-yellow-900' : 'bg-white border-slate-100 text-slate-300'}`}
                >
                  {d.label[0]}
                </button>
              ))}
